...
class EditShot(BaseModel):
    ...
    @classmethod
    def validate_trim_end_gt_start(cls, v, info):
        """Ensure trim_end is greater than trim_start."""
        if "trim_start" in info.data and v <= info.data["trim_start"]:
            raise ValueError("trim_end must be greater than trim_start")
        return v

    @field_validator("audio_start_offset")
    @classmethod
    def validate_audio_offset_rules(cls, v, info):
        """Validate audio offset rules based on edit_type."""
        if "edit_type" in info.data:
            edit_type = info.data["edit_type"]
